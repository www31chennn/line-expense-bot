...
function budgetOverviewFlex(result) {
  ...
  // 分類的金額都是用比例算出來的（monthlyLimit × 比例），比例總和不是100%的話，
  // 下面顯示的各分類金額/警示會失準——不到100%代表有分類還沒分到；超過100%代表
  // 各分類上限加起來比實際月預算還多，兩種都提醒一下，不特別說要去哪裡改（按鈕在別的卡片上）
  ...
  if (activePctSum < 100) {
    headerContents.push({
      type: 'text',
      text: `分類比例目前只分配了 ${activePctSum}%，下面的金額可能不準`,
      color: '#F5D998',
      size: 'xxs',
      wrap: true,
      margin: 'xs',
    });
  } else if (activePctSum > 100) {
    headerContents.push({
      type: 'text',
      text: `分類比例加起來 ${activePctSum}%，超過100%了，下面的金額可能不準`,
      color: '#F5D998',
      size: 'xxs',
      wrap: true,
      margin: 'xs',
...
function buildAllocationRows(allocation, monthlyLimit, categories) {
  ...
  // 反過來也可能總和超過100%（例如用「飲食大約花X元」換算比例套用時沒有擋超額）。
  // 兩種情況比例總和都不是100%，長條圖上的個別數字還是準的，但加起來對不上，一併提醒
  const usedSum = categories.reduce((sum, cat) => sum + Math.round(allocation[cat.name] ?? 0), 0);
  const unallocated = Math.max(0, 100 - usedSum);
  let gapNote = null;
  if (unallocated > 0) {
    gapNote = `還有 ${unallocated}% 還沒分配，別忘了到「設定所有比例」補上喔～`;
    rows.push({
      type: 'box',
      layout: 'vertical',
      margin: 'md',
  ...
  if (usedSum > 100) {
    gapNote = `分類比例加起來 ${usedSum}%，超過100%了`;
  }
  return { rows, gapNote };
}
...
function categoryAllocationFlex(headerTitle, headerSubtitle, allocation, monthlyLimit, categories) {
  ...
  if (subtitle) {
    headerContents.push({ type: 'text', text: subtitle, color: '#F5D998', size: 'xs', wrap: true, margin: 'sm' });
  }
  ...
  const menuSections = [
    ...
    { icon: '💰', title: '預算', desc: '查看本月已花金額、剩餘額度與各分類佔比。', example: '範例：「這個月還剩多少」' },
    {
      ...
      subsections: [
        ...
        {
          title: '📊 修改比例',
          desc: '調整各分類的預算比例。改比例統一用網頁設定，可以一次看到所有分類，調整到100%即可存檔。',
...
function categoryPercentHelpFlex(allocation, monthlyLimit, categories) {
  ...
  if (gapNote) {
    headerContents.push({ type: 'text', text: gapNote, color: '#F5D998', size: 'xs', wrap: true, margin: 'sm' });
  }
...
export function resultToLineMessages(result) {
  switch (result.type) {
    ...
    case 'calc_category_pct': {
      ...
      const liffItem = process.env.NEXT_PUBLIC_LIFF_ID
        ? {
            type: 'action',
            action: {
              type: 'uri',
      ...
      if (liffItem) items.push(liffItem);
    ...
    case 'calc_category_pct_confirmed': {
      ...
      if (result.invalid) {
        return [{ type: 'text', text: `⚠️「${result.category}」目前不是啟用中的分類，可能中途被停用了，請重新查詢` }];
      }
      const liffButton = process.env.NEXT_PUBLIC_LIFF_ID
        ? [
            {
              type: 'action',
              action: {
      ...
      // 套用一律直接寫入，其他分類完全不動；套用後才知道總和有沒有超過100%，
      // 超過的話這裡才提示超額多少，引導去網頁調整——不在套用前擋
      if (result.overLimit) {
        const over = result.resultingTotal - 100;
        const overMessage = {
          type: 'text',
          text: `✅ 已將「${result.category}」設為 ${result.pct}%\n⚠️ 但所有分類加起來變成 ${result.resultingTotal}%，超過了 ${over}%，各分類的金額可能會不準`,
        };
        if (liffButton.length > 0) overMessage.quickReply = { items: liffButton };
        return [overMessage];
      }
      ...
      const message = {
        type: 'text',
        text:
          result.resultingTotal < 100
            ? `✅ 已將「${result.category}」設為 ${result.pct}%，還差 ${100 - result.resultingTotal}% 到100%，別忘了到「設定所有比例」補上喔～`
            : `✅ 已將「${result.category}」設為 ${result.pct}%`,
      };
      if (process.env.NEXT_PUBLIC_LIFF_ID) {
        message.quickReply = { items: liffButton };
      }
      return [message];
    }
    ...
    case 'disable_category': {
      ...
      const lines = result.results.map((r) => {
        ...
        return r.reclaimedPct > 0
          ? `✅ 已停用「${r.category}」，${r.reclaimedPct}% 還沒分配，別忘了到「設定所有比例」補上喔～`
          : `✅ 已停用「${r.category}」`;
      });
    ...
    case 'enable_category': {
      ...
      const lines = result.results.map((r) => {
        ...
        return `✅ 已啟用「${r.category}」，目前 0%，別忘了到「設定所有比例」補上喔～`;
      });
      const message = { type: 'text', text: lines.join('\n') };
      if (result.results.some((r) => r.ok) && process.env.NEXT_PUBLIC_LIFF_ID) {
        message.quickReply = {
