  ...
  return {
    ...
    contents: {
      ...
      header: {
        ...
        backgroundColor: '#5B7F76',
        paddingAll: 'lg',
        contents: [{ type: 'text', text: '🟢 啟用中的分類', color: '#ffffff', size: 'lg', weight: 'bold' }],
      },
      body: { type: 'box', layout: 'vertical', paddingAll: 'lg', contents: rows },
    },
  };
...
function categorySettingsDisabledFlex(disabled) {
  ...
  const message = {
    type: 'flex',
    altText: `已停用的分類（共 ${disabled.total} 個）`,
    contents: {
      type: 'bubble',
      header: {
        type: 'box',
        ...
        paddingAll: 'lg',
        contents: [
          { type: 'text', text: '⚪ 已停用的分類', color: '#ffffff', size: 'lg', weight: 'bold' },
          { type: 'text', text: `共 ${disabled.total} 個`, color: '#eeeeee', size: 'xs' },
        ],
      },
      body: { type: 'box', layout: 'vertical', paddingAll: 'lg', contents: rows },
    },
...
export function resultToLineMessages(result) {
  switch (result.type) {
    ...
    case 'rename_category': {
      ...
      if (result.notFound) return [{ type: 'text', text: `⚠️ 找不到「${result.requested}」這個分類` }];
      if (result.invalid) return [{ type: 'text', text: '⚠️ 請提供新的分類名稱' }];
      if (result.tooLong) {
        return [{ type: 'text', text: `⚠️ 分類名稱太長了，請控制在 ${result.maxLength} 個字以內` }];
      }
      if (result.duplicate) return [{ type: 'text', text: `⚠️「${result.newName}」已經存在了，不能重複使用` }];
