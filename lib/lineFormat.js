...
function candidateListFlex(title, candidates, headerColor = '#5B7F76') {
  const rows = [];
  ...
  return {
    ...
    contents: {
      ...
      body: {
        ...
        spacing: 'md',
        contents: [
          { type: 'text', text: '跟我說薪水跟目標，我就會算出每月可花上限：', size: 'sm', wrap: true, color: '#555555' },
          {
            type: 'box',
            layout: 'vertical',
          ...
          {
            type: 'text',
            text: '之後想改分類比例可以說「修改飲食為30%」，一次改多個也行，例如「飲食30%，交通10%」；或用下面按鈕調整。',
            size: 'xs',
            wrap: true,
            color: '#999999',
            margin: 'md',
...
export function resultToLineMessages(result) {
  switch (result.type) {
    ...
    case 'manage_unspecified': {
      ...
      message.quickReply = quickReplyFromIndexed(result.candidates);
      return [message];
    }
    ...
    case 'budget_help': {
      const message = budgetHelpFlex();
      message.quickReply = {
        items: [
          {
            type: 'action',
            action: { type: 'message', label: '🎯 用按鈕調整比例', text: '調整分類比例' },
          },
        ],
      };
      return [message];
