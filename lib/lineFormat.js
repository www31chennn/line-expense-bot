const ALL_CATEGORIES_LOCAL = ['飲食', '交通', '購物', '娛樂', '醫療', '居家', '固定支出', '其他'];
  ...
  return {
    ...
    contents: {
      ...
      body: {
        ...
        contents: [
          statusBox,
          { type: 'text', text: '提供薪水，系統會自動計算每月可花費上限；存款目標非必填：', size: 'sm', wrap: true, color: '#555555' },
          {
            type: 'box',
            layout: 'vertical',
...
export function resultToLineMessages(result) {
  switch (result.type) {
    ...
    case 'manage_unspecified': {
      ...
      const message = candidateListFlex(header, result.candidates, '#B8860B');
    ...
    case 'budget_help': {
      ...
      const items = [
        ...
        { type: 'action', action: { type: 'message', label: '🔄 恢復預設比例', text: '恢復預設比例' } },
      ];
    ...
    case 'view_category_allocation': {
      const message = categoryAllocationFlex('📊 目前分類比例', null, result.allocation, result.monthlyLimit);
      message.quickReply = {
        items: [{ type: 'action', action: { type: 'message', label: '🔄 恢復預設比例', text: '恢復預設比例' } }],
      };
      return [message];
    }

    case 'category_allocation_reset':
      return [categoryAllocationFlex('✅ 已恢復預設分類比例', null, result.allocation, result.monthlyLimit)];
    ...
    case 'set_category_budget': {
      ...
      if (result.allSpecifiedMismatch) {
        return [
          {
            type: 'text',
            text: `⚠️ 你一次講了全部8個分類，但加起來是 ${result.specifiedSum}%，不是100%，因為沒有其他分類可以自動吸收差額，麻煩調整成剛好100%`,
          },
        ];
      }
