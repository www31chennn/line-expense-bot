...
function monthlyReportFlex(result) {
  ...
  const bodyContents = result.categories.map((c) => ({
    type: 'box',
    layout: 'vertical',
    margin: 'md',
    spacing: 'xs',
    contents: [
      {
        ...
        contents: [
          { type: 'text', text: `${CATEGORY_EMOJI[c.category] || ''} ${c.category}`, size: 'sm', flex: 3 },
          { type: 'text', text: `$${c.amount}（${c.percentage}%）`, size: 'xs', color: '#999999', align: 'end', flex: 4 },
        ],
      },
      {
        type: 'box',
        ...
        backgroundColor: '#EAEAEA',
        contents: [
          { type: 'box', layout: 'vertical', flex: Math.max(1, Math.round(c.percentage)), backgroundColor: '#5B7F76', contents: [] },
          { type: 'box', layout: 'vertical', flex: Math.max(1, 100 - Math.round(c.percentage)), contents: [] },
        ],
      },
    ],
  }));
