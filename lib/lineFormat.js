...
export function resultToLineMessages(result) {
  switch (result.type) {
    ...
    case 'list': {
      ...
      if (result.hasMore) {
        message.quickReply = {
          items: [{ type: 'action', action: { type: 'message', label: '看更多 ↓', text: '看更多' } }],
        };
      }
      return [message];
    }
    ...
    case 'budget_status':
      return [budgetOverviewFlex(result)];
    ...
    case 'query_category_budget':
      return [budgetOverviewFlex({ notSet: result.monthlyLimit == null, month: result.month, categories: result.table })];
