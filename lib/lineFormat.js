    ...
    contents: {
      ...
      body: {
        ...
        contents: [
          ...
          {
            type: 'text',
            text: '把每個月可以花的錢分給各分類，例如飲食佔30%，全部加起來是100%：',
            size: 'xs',
            wrap: true,
            color: '#555555',
            margin: 'xs',
          ...
          {
            type: 'text',
            text: '改了其中一個，其他分類我會自動幫你調整，總和維持100%。',
            size: 'xxs',
            wrap: true,
            color: '#999999',
            margin: 'sm',
  ...
  switch (result.type) {
    ...
    case 'list': {
      if (result.records.length === 0) {
        ...
        let text = result.category
          ? `📋 沒有符合條件的「${result.category}」記錄`
          : '📋 沒有符合條件的記錄';
        // 這個分類雖然沒有花費記錄，但預算卡上有分配比例：主動解釋兩種 % 的差異，
        // 避免使用者把「分配比例」誤會成「已花費佔比」而追問
        if (result.emptyCategoryAllocationPct != null && result.category) {
          text += `\n\n💡 補充：預算卡上「${result.category}」顯示的 ${result.emptyCategoryAllocationPct}% 是你設定的預算「分配比例」（這個分類最多可以花月上限的多少），不是已花費的佔比，所以就算沒有任何記錄也會顯示比例。`;
        }
        return [{ type: 'text', text }];
      }
