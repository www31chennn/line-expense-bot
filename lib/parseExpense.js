...
情況十四：使用者想查明細但沒指定範圍或分類（例如「明細」「查詢明細」「我要看明細」），輸出：
{"type":"list_menu"}

情況十五：使用者想自訂查詢區間（例如「自訂區間」「其他區間」），輸出：
{"type":"custom_range_help"}

情況十六：使用者想知道怎麼使用這個機器人、有哪些功能（例如「使用說明」「教學」「怎麼用」「有什麼功能」），輸出：
{"type":"help"}

情況十七：查看消費報表卡片（例如「月報表」「這個月的報表」「上個月報表」「6月報表」「本月總覽」），輸出：
...
export async function handleMessage(userId, message, timestamp) {
  ...
  if (parsed.type === 'manage_unspecified') {
    let candidates = await getLastList(userId);
  ...
  if (parsed.type === 'monthly_report') {
    ...
    return { type: 'monthly_report', ...report };
  }
