...
- 「這個月」指當月 1 號到今天；「這週」「本週」指這週一到今天

情況三：列出清單（例如「列出所有飲食」「列出這個月醫療」「列出交通類」「列出今天所有記錄」「列出這週所有記錄」「本月明細」），輸出：
{"type":"list","category":"分類或null","startDate":"YYYY-MM-DD或null","endDate":"YYYY-MM-DD或null"}
- 沒指定日期範圍就給 null（不限制）；「今天」startDate=endDate=今天；「這個月」「本月」指當月1號到今天；「這週」「本週」指這週一到今天
...
情況十一：查詢預算/目標狀態（例如「這個月還剩多少可以花」「有沒有超支」「存錢目標達成了嗎」「餘額還有多少」），輸出：
{"type":"budget_status"}
...
情況十三：使用者想知道怎麼設定預算，但沒有直接給數字（例如「設定預算」「怎麼設定薪水」「預算怎麼設定」），輸出：
{"type":"budget_help"}
...
export async function handleMessage(userId, message, timestamp) {
  ...
  if (pendingBefore && pendingBefore.action === 'choose_action') {
    ...
    if (trimmed.includes('編輯') || trimmed.includes('改')) {
    ...
    if (trimmed.includes('刪除') || trimmed.includes('刪')) {
      const doc = await recordsCollection(userId).doc(targetId).get();
      await clearPendingAction(userId);
  ...
  if (parsed.type === 'manage_unspecified') {
    ...
    return { type: 'manage_unspecified', candidates: recent };
  }
