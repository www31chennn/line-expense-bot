...
// 這筆比例要怎麼用由使用者自己決定（通常是去「修改比例」網頁重新分配），
// 不是被系統偷偷塞給剩下的分類。回傳收回的比例讓呼叫端可以提示使用者
...
export async function handleMessage(userId, message, timestamp) {
  ...
  // 但 confirm_calc_pct 是「等使用者按確認鈕」的暫存，不算換話題，不清掉
  if (parsed.type !== 'select_index' && (!pendingBefore || pendingBefore.action !== 'confirm_calc_pct')) {
    await clearPendingAction(userId);
  }
  ...
  if (parsed.type === 'calc_category_pct') {
    ...
    // 先算出比例、存進 pendingAction 等使用者確認，不立刻寫入
    // 確認後走 confirm_calc_pct postback 再真正呼叫 setCategoryBudgets
    await savePendingAction(userId, { action: 'confirm_calc_pct', category: matched, pct, monthlyLimit: budget.monthlyLimit });
    return {
