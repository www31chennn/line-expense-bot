...
- reply 的寫法規則（依序判斷）：
  ...
  2. 使用者在問功能怎麼用、或某個數字/機制的「意義」的話，「只能」依下面的功能總覽說明，總覽沒涵蓋的請他輸入「使用說明」，嚴禁自行推測機制。另外你看不到這位使用者的任何記帳資料與設定，嚴禁提及或編造任何具體金額、筆數、百分比數字
     ...
     - 分配比例：分類旁的%是「預算分配比例」（這個分類可以花上限的多少），月報表上的%才是實際花費佔比；比例不用設薪水就存在——一開始所有啟用分類均分，會因「調整比例」「新增分類」「停用分類」而變動；總和固定100%，改一個其他一定跟著變、沒有鎖住功能，想精確控制可以一次指定多個（「飲食30%，交通20%」），只有沒指定的會自動調整；還沒設上限時比例照樣顯示，只是還沒有對應金額
     - 分類：內建8個＋可自訂，最多同時啟用12個；「新增分類」可加新的（名稱最長6個字、可附emoji）；停用不是刪除、隨時可再啟用，停用時它的比例會回收分給其他分類；自訂分類可以改名/換emoji，內建分類的emoji是固定的
...
export async function handleMessage(userId, message, timestamp) {
  ...
  if (parsed.type === 'list') {
    ...
    // 指定分類卻查到 0 筆時，常見的誤會是把預算卡上的「分配比例」（配額）當成「已花費佔比」，
    // 然後追問「沒記錄為什麼還有X%」。這裡把該分類目前的分配比例一起帶回去，
    // 讓 lineFormat 在回覆「沒有記錄」的同時主動解釋那個 % 的意義，在誤會發生的當下就講清楚
    if (result.count === 0 && category && activeNames.includes(category)) {
      const budget = await getBudget(userId);
      const allocation = normalizeAllocation(budget && budget.categoryAllocation, activeNames);
      const pct = allocation[category] || 0;
      return { type: 'list', ...result, emptyCategoryAllocationPct: pct > 0 ? pct : null };
    }
