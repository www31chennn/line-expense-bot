...
// 固定的 8 個分類，反問使用者時用這組當選項
const ALL_CATEGORIES = ['飲食', '交通', '購物', '娛樂', '醫療', '居家', '固定支出', '其他'];

// 如果帳號是在新增「固定支出」這個分類之前就設定過比例，存在 Firestore 的舊資料會缺這個 key。
// 這裡統一補齊：缺的分類給 0%，不動使用者已經自訂過的其他分類數值，也不會偷偷幫使用者重新分配
function normalizeAllocation(stored) {
...
- date 從語句推算絕對日期；category 從 ["飲食","交通","購物","娛樂","醫療","居家","固定支出","其他"] 選一個最可能的
...
- 「固定支出」這個分類專門給每月固定要繳、金額通常不變的支出用，例如房貸、房租、保險費、分期付款、訂閱服務（Netflix等）；不是每月都有、金額會變動的（例如水電費、電話費）還是照原本的分類判斷（居家、其他等）
...
- category 必須是 ["飲食","交通","購物","娛樂","醫療","居家","固定支出","其他"] 其中一個
...
情況十四之一：使用者想把分類比例恢復成系統預設值（例如「恢復預設比例」「重設分類比例」「分類比例回復預設」），輸出：
{"type":"reset_category_allocation"}

情況十五：使用者想知道怎麼設定預算，但沒有直接給數字（例如「設定預算」「怎麼設定薪水」「預算怎麼設定」），輸出：
{"type":"budget_help"}

情況十五之一：使用者想刪除/清除/重設薪水與目標的設定（例如「刪除預算」「清除預算目標」「重設預算」「取消預算設定」），輸出：
{"type":"delete_budget"}
...
- category 從 ["飲食","交通","購物","娛樂","醫療","居家","固定支出","其他"] 選一個
...
async function listRecentRecords(userId, limit) {
  ...
  // 挑選「哪 N 筆」還是照最近建立的（符合「編輯剛打的」的直覺），
  // 但顯示順序改成照消費日期排，讓清單好找、好對照
  records.sort((a, b) => a.date.localeCompare(b.date) || a.amount - b.amount);
...
// 一次調整一到多個分類，沒講到的分類依原本的相對比例自動補滿剩下的%
async function setCategoryBudgets(userId, adjustments) {
  const existingBudget = (await getBudget(userId)) || {};
  const current = normalizeAllocation(existingBudget.categoryAllocation);

  // 檢查分類名稱都合法
  const invalidCategory = adjustments.find((a) => !(a.category in current));
  ...
  const otherKeys = ALL_CATEGORIES.filter((cat) => !specifiedCategories.includes(cat));
  const remaining = 100 - specifiedSum;

  // 如果全部 8 個分類都講了，沒有剩下的分類可以吸收差額，加起來必須剛好 100%
  ...
  // 依固定順序（ALL_CATEGORIES）輸出，不要用物件鍵值順序
  const ordered = {};
  ALL_CATEGORIES.forEach((cat) => {
...
// 把分類比例恢復成系統預設值，不影響薪水/目標/每月上限這些其他設定
async function resetCategoryAllocation(userId) {
  const existingBudget = (await getBudget(userId)) || {};
  const updatedBudget = { ...existingBudget, categoryAllocation: { ...DEFAULT_CATEGORY_ALLOCATION } };
  await userDoc(userId).set({ budget: updatedBudget }, { merge: true });
...
async function getCategoryBudgetStatus(userId) {
  const budget = await getBudget(userId);
  const allocation = normalizeAllocation(budget && budget.categoryAllocation);
...
export async function handleMessage(userId, message, timestamp) {
  ...
  if (pendingBefore && pendingBefore.action === 'choose_action') {
    ...
    if (trimmed === '刪除' || trimmed === '刪' || trimmed === '要刪除') {
      ...
      await removeFromLastList(userId, targetId);
  ...
  if (parsed.type === 'manage_unspecified') {
    ...
    return { type: 'manage_scope_prompt', hasLastList: lastList.length > 0 };
  }
  ...
  if (parsed.type === 'reset_category_allocation') {
    const allocation = await resetCategoryAllocation(userId);
    const budget = await getBudget(userId);
    return { type: 'category_allocation_reset', allocation, monthlyLimit: budget ? budget.monthlyLimit : null };
  }
  ...
  if (parsed.type === 'delete_budget') {
    const existing = await getBudget(userId);
    if (!existing || existing.monthlyLimit == null) {
      return { type: 'delete_budget', wasEmpty: true };
    }
    await clearBudget(userId);
    return { type: 'delete_budget', wasEmpty: false };
  }

  if (parsed.type === 'view_category_allocation') {
    const catStatus = await getCategoryBudgetStatus(userId);
    return { type: 'view_category_allocation', allocation: catStatus.allocation, monthlyLimit: catStatus.monthlyLimit };
  }
