...
{"type":"query","startDate":"YYYY-MM-DD","endDate":"YYYY-MM-DD","category":"分類或null","label":"期間描述，例如：今天、這個月"}
- 「這個月」指當月 1 號到今天

情況三：列出清單（例如「列出所有飲食」「列出這個月醫療」「列出交通類」「列出今天所有記錄」「本月明細」），輸出：
{"type":"list","category":"分類或null","startDate":"YYYY-MM-DD或null","endDate":"YYYY-MM-DD或null"}
- 沒指定日期範圍就給 null（不限制）；「今天」startDate=endDate=今天；「這個月」「本月」指當月1號到今天
...
情況八：使用者要編輯，但完全沒提到日期/品項/分類/編號（例如「我要編輯」「我想改一筆」「編輯記錄」），輸出：
{"type":"edit_unspecified"}

情況九：使用者要刪除，但完全沒提到日期/品項/分類/編號（例如「我要刪除一筆」「我想刪東西」），輸出：
{"type":"delete_unspecified"}

情況十：使用者只是單獨回覆一個編號來回答「要選哪一筆」（例如「1」「#1」「第一筆」「第二個」「2」，沒有其他記帳/查詢內容），輸出：
{"type":"select_index","index":數字}
- 中文數字（一二三四五）、阿拉伯數字、「#1」「第1筆」等格式都要能轉換成數字

情況十一：設定薪水與存錢/花費目標（例如「薪水50000，目標存15000」「月薪45000，最多花70%」「我想每個月存1萬」），輸出：
{"type":"set_budget","salary":數字或null,"savingsGoal":數字或null,"spendingPercentage":數字或null}
- 只填使用者實際提到的欄位，沒提到的給 null

情況十二：查詢預算/目標狀態（例如「這個月還剩多少可以花」「有沒有超支」「存錢目標達成了嗎」「餘額還有多少」），輸出：
{"type":"budget_status"}

情況十三：修改某分類的預算比例（例如「修改飲食為30%」「交通改成15%」），輸出：
{"type":"set_category_budget","category":"分類","percentage":數字}
- category 必須是 ["飲食","交通","購物","娛樂","醫療","居家","其他"] 其中一個

情況十四：查詢分類預算配置（例如「各分類預算是多少」「飲食可以花多少」），輸出：
{"type":"query_category_budget"}

情況十五：匯出/總覽報表卡片（例如「匯出報表」「這個月的報表」「幫我做本月總覽」），輸出：
{"type":"export_report"}

情況十六：完全無法辨識意圖，輸出：
{"type":"none"}

判斷「上一筆」還是「特定一筆」的關鍵：只要句子裡有提到日期、品項名稱、分類、或「第X筆」，一律當作「特定一筆」（情況六/七），只有完全沒提到任何指向性資訊、單純講「剛剛」「上一筆」時才用情況四/五；如果連「剛剛」「上一筆」都沒講，只是單純說「我要編輯/刪除」，用情況八/九。單獨的編號回覆（無其他內容）一律用情況十。
...
// 把清單存起來，讓「第2筆」這種指代之後可以查回對應的 record id
async function saveLastList(userId, indexedRecords) {
  await userDoc(userId).set(
    { lastList: indexedRecords.map((r) => ({ index: r.index, id: r.id })) },
    { merge: true }
  );
}
...
async function listRecords(userId, category, startDate, endDate, offset = 0) {
  ...
  const hasMore = offset + LIST_DISPLAY_LIMIT < fullCount;
  return { records: indexed, total: fullTotal, count: fullCount, offset, hasMore };
}

// 記住這次列表的查詢條件跟目前看到哪，讓「看更多」可以接著撈下一批
async function saveListQuery(userId, query) {
  await userDoc(userId).set({ listQuery: query }, { merge: true });
}

async function getListQuery(userId) {
  const doc = await userDoc(userId).get();
  if (!doc.exists) return null;
  return doc.data().listQuery || null;
}
...
async function getCategoryBudgetStatus(userId) {
  ...
  const table = Object.entries(allocation).map(([cat, pct]) => {
...
export async function handleMessage(userId, message, timestamp) {
  // 「看更多」是 Quick Reply 按鈕送出的固定文字，直接處理不用跑 AI 分類
  if (message.trim() === '看更多') {
    const listQuery = await getListQuery(userId);
    if (!listQuery) return { type: 'none' };
    const nextOffset = (listQuery.offset || 0) + LIST_DISPLAY_LIMIT;
    const result = await listRecords(userId, listQuery.category, listQuery.startDate, listQuery.endDate, nextOffset);
    await saveListQuery(userId, { ...listQuery, offset: nextOffset });
    return { type: 'list', ...result, category: listQuery.category };
  }
  ...
  if (parsed.type === 'select_index') {
    ...
    if (pendingBefore.action === 'select_target') {
      // 選好要編輯哪一筆了，記住這筆，改問要改成什麼
  ...
  if (parsed.type === 'list') {
    ...
    const result = await listRecords(userId, category, startDate, endDate, 0);
    await saveListQuery(userId, { category, startDate, endDate });
    return { type: 'list', ...result, category };
  }
  ...
  if (parsed.type === 'edit_unspecified') {
    const recent = await listRecentRecords(userId, 10);
    await savePendingAction(userId, { action: 'select_target' });
    return { type: 'edit_unspecified', action: 'modify', candidates: recent };
  }

  if (parsed.type === 'delete_unspecified') {
    const recent = await listRecentRecords(userId, 10);
    await savePendingAction(userId, { action: 'delete' });
    return { type: 'edit_unspecified', action: 'delete', candidates: recent };
  }
  ...
  if (parsed.type === 'query_category_budget') {
    const status = await getCategoryBudgetStatus(userId);
    return { type: 'query_category_budget', ...status };
  }

  if (parsed.type === 'export_report') {
    const { dateStr } = getTodayInfo(timestamp);
    const month = dateStr.slice(0, 7);
    const report = await getMonthlyCategoryBreakdown(userId, month);
    const budget = await getBudget(userId);
    return { type: 'export_report', ...report, monthlyLimit: budget ? budget.monthlyLimit : null };
  }
