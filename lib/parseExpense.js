...
// 列出符合條件的記錄，依日期、金額排序並附上編號
async function listRecords(userId, category, startDate, endDate) {
...
export async function handleMessage(userId, message, timestamp) {
  ...
  if (parsed.type === 'list') {
    const records = await listRecords(userId, parsed.category || null, parsed.startDate || null, parsed.endDate || null);
    const total = records.reduce((s, r) => s + r.amount, 0);
    return { type: 'list', records, total, category: parsed.category || null };
  }
  ...
  if (parsed.type === 'budget_status') {
    ...
    if (!status) return { type: 'budget_status', notSet: true };
    return { type: 'budget_status', ...status };
  }
