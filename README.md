...
### 6.5 之後要做的：Rich Menu（常駐選單）

目前互動已經支援 **Quick Reply**（分類反問、選第幾筆這種一次性按鈕，跟著訊息
出現又消失），但你截圖問的「常駐在輸入框上方的深藍色按鈕」是另一個功能
**Rich Menu**，需要：

1. 設計一張選單圖片（LINE 官方尺寸建議 2500x1686 或 2500x843）
2. 用 Rich Menu API 上傳圖片、設定每個區塊對應的座標範圍跟動作（通常是送出固定文字）
3. 把它設成該帳號的預設選單

這塊還沒做，因為需要你先決定要放哪幾個按鈕、版面想怎麼排，圖片也需要設計。
確定圖片之後我可以幫你寫上傳跟綁定的程式碼。