...
- emoji 欄位只接受剛好一個 emoji 符號（用 `Intl.Segmenter` 切字素叢集判斷，支援像 🏃‍♂️、👨‍👩‍👧‍👦
...
### 8.1 LINE Official Account Manager 網頁後台（不用寫程式）

1. 登入 [manager.line.biz](https://manager.line.biz)
2. 左側選單「群發訊息」-> 「建立」
3. 訊息格式限定文字、圖片、影片、優惠券、或 LINE 自己的「多頁訊息」樣板（用他們的視覺化編輯器排版）；
   **這個後台沒辦法貼自訂的 Flex Message JSON**，要自訂版面（像分類設定那種卡片）只能走 8.2 的 API
4. 設定發送對象、時間，送出

### 8.2 用 API 腳本發（可以重用專案裡的 Flex 卡片視覺風格）

`scripts/broadcast.js` 已經寫好一份範例（新功能上線：分類設定），跟 `setup-rich-menu.js`
同一套模式，改公告內容直接改腳本裡的 `announcement` 物件即可：

```bash
# 群發給全部好友
npm run broadcast

# 只推播給指定一個人（測試用，不算進群發額度）
npm run broadcast -- --to <userId>
```

`<userId>` 要用真實 LINE userId（`U` 開頭一長串），不是暱稱；可以到 Firestore 的
`expenses` collection 底下找——只要那個人用 LINE 跟機器人講過話，文件 ID 就是他的 userId。
用 `--to` 測試的前提是那個 userId 已經加你的官方帳號好友（Push API 沒辦法推播給非好友）。

- LINE 免費（輕用量）方案每月有 **200 則群發免費額度**，算法是「收到的好友數」不是「呼叫次數」
  （1 個好友收到算 1 則）；`--to` 單一推播不算在群發額度裡，是算在另一個「訊息推播」額度，通常寬鬆很多
- 全體群發沒有分眾功能，會送給「全部」好友；正式群發前建議先用 `--to` 測試效果
- 一次最多送 5 則訊息（文字/Flex 皆可混合），執行下去無法收回，改完內容再執行